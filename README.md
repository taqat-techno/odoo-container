# Odoo 14 Enterprise — Docker Container

Branch `v14` of `taqat-techno/odoo-container`.

This repository contains the **Odoo 14 Enterprise source code** pre-configured for Docker-based development. It is designed to work with a shared Docker image hosted on Docker Hub — no local image build required.
...
```
odoo-container/ (v14 branch)
├── odoo/                    ← Odoo 14 Enterprise source (standard addons)
├── conf/
...
```bash
git clone -b v14 https://github.com/taqat-techno/odoo-container.git my-project-odoo14
cd my-project-odoo14
```
...
# Enable dev mode (auto-reload)
# Set DEV_MODE=1 in .env, then restart
```
