...
class SIE4ImportWizard(models.TransientModel):
    ...
    def _import_key_konto(self, data_map, account_code, account_name):
        ...
        if account_code in data_map['existing_accounts_map']:
            ...
            data_map['account.account'].setdefault(account_xml_id, {'company_id': self.company_id.id, 'code': account_code})
    ...
    def _prepare_sie4_opening_balance_move(self, data_map):
        ...
        account_sum_group = self.env['account.move.line']._read_group(
            domain=[
                *self.env['account.move.line']._check_company_domain(self.company_id),
        ...
        if unseen_account_codes := set(account_codes).difference(group_account.code for group_account, _account_sum in account_sum_group):
            unseen_accounts = self.env['account.account'].search_fetch(
                domain=[
                    *self.env['account.account']._check_company_domain(self.company_id),
                    ('code', 'in', tuple(unseen_account_codes)),
                ],
                field_names=['id', 'code'],
            )
        ...
        if not currency.is_zero(total_diff):
            off_balance_account = self.env['account.account'].search_fetch([
                *self.env['account.account']._check_company_domain(self.company_id),
    ...
    def action_import_sie4(self):
        ...
        existing_accounts = self.env['account.account'].search_fetch(
            domain=self.env['account.account']._check_company_domain(self.company_id),
            field_names=['id', 'code'],
        )
        ...
        data_map = {
            ...
            'existing_accounts_map': {account.code: account.id for account in existing_accounts},
        ...
        data = self.env['account.chart.template']._load_data(create_data)

        # Set the sequence of the newly imported moves
        if moves := data.get('account.move'):
            moves = moves.sorted(lambda m: (m.date, m.ref or '', m._origin.id))
            for move in moves:
                if move.name == '/':
                    move._set_next_sequence()

        # TODO from saas-17.2+: use import summary wizard to display the imported data
        return {
