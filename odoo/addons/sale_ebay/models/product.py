...
class ProductTemplate(models.Model):
    ...
    def check_encoding(self, barcode, encoding):
        if encoding == 'ean13':
            return len(barcode) == 13 and re.match(r"^\d+$", barcode) and self.ean_checksum(barcode) == int(barcode[-1])
        elif encoding == 'upc':
            return len(barcode) == 12 and re.match(r"^\d+$", barcode) and self.ean_checksum("0"+barcode) == int(barcode[-1])
        elif encoding == 'any':
            return True
        else:
            return False
