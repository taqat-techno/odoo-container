...
class ProductTemplate(models.Model):
    ...
    base_unit_price = fields.Monetary("Price Per Unit", currency_field="currency_id",
    ...
    def _get_sales_prices(self, pricelist):
        ...
        for template in self:
            ...
            base_price = None
            price_list_contains_template = pricelist.currency_id.compare_amounts(price_reduce,
                                                                                 base_sales_prices[template.id]) != 0
    ...
    def _get_combination_info(self, combination=False, product_id=False, add_qty=1, pricelist=False,
        ...
        if self.env.context.get('website_id'):
            ...
            if pricelist and pricelist.currency_id != product.currency_id:
                compare_list_price = self.currency_id._convert(self.compare_list_price, pricelist.currency_id,
                                                               self.env.company,
