...
class TestPayslipComputation(TestPayslipContractBase):
    ...
    def test_work_data_with_exceeding_interval(self):
        ...
        entry_exceeding_lower_bound.unlink()
        entry_exceeding_upper_bound = self.env['hr.work.entry'].create({
            'name': 'Attendance',
            'employee_id': self.richard_emp.id,
            'contract_id': self.contract_cdd.id,
            'work_entry_type_id': self.env.ref('hr_work_entry.work_entry_type_attendance').id,
            'date_start': datetime(2015, 12, 13, 18, 0),  # 19:00 local time
            'date_stop': datetime(2015, 12, 14, 2, 0),    # 03:00 local time
        })
        ...
        entry_exceeding_upper_bound.unlink()  # For symmetry, unlink this one too.
    ...
    def test_payslip_multiple_inputs_and_attachments_same_type(self):
        ...
        with Form(payslip) as payslip_form:
            payslip_form.input_line_ids.remove(0)
            with payslip_form.input_line_ids.new() as line:
                line.input_type_id = self.env.ref("hr_payroll.input_attachment_salary")
                line.amount = 100
            with payslip_form.input_line_ids.new() as line:
                line.input_type_id = self.env.ref("hr_payroll.input_attachment_salary")
                line.amount = 200
