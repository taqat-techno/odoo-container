...
class TestFrenchTaxClosing(TestAccountReportsCommon):
    ...
    def setUpClass(cls, chart_template_ref='fr'):
        ...
        cls.report = cls.env.ref('l10n_fr.tax_report')
    ...
    def test_fr_send_edi_vat_values_vat_reimbursed_by_administration(self):
        ...
        edi_vals = send_vat_wizard.with_context({'l10n_fr_generation_options': options})._prepare_edi_vals(options)
        ...
        with patch.object(self.env.registry['l10n_fr_reports.send.vat.report'], '_send_xml_to_aspone', return_value=[]):
            send_vat_wizard.with_context({'l10n_fr_generation_options': options}).send_vat_return()
            report_line_26 = self.env.ref('l10n_fr.tax_report_26_external')
            line_26_values = next(
    ...
    def test_fr_send_empty_declaration_to_administration(self):
        ...
        def get_edi_vals(start_date, end_date):
            send_vat_wizard = self.env['l10n_fr_reports.send.vat.report'].create({'test_interchange': True})
            options = self._generate_options(
                self.report,
            ...
            return send_vat_wizard.with_context({'l10n_fr_generation_options': options})._prepare_edi_vals(options)

        self.env['account.move'].create([
            self._get_move_create_data(
                move_data={'move_type': 'out_invoice', 'invoice_date': '2024-05-08', 'journal_id': self.company_data['default_journal_sale'].id},
                line_data={'price_unit': 100, 'tax_ids': [Command.link(self.tax_20_g_sale.id)]}
            ),
        ])._post()
