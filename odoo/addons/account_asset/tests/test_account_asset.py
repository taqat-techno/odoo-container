...
class TestAccountAsset(TestAccountReportsCommon):
    ...
    def test_account_assets_report_multi_companies(self):
        """ Test the Depreciation Schedule report when the assets are not sorted by company. """

        # 2 accounts from different companies sharing the same code
        account_1, account_2, *_ = self.env['account.account'].search([('code', '=', '101000')])

        company_1, company_2 = account_1.company_id, account_2.company_id
        currency = company_1.currency_id
        asset_vals = {'asset_type': 'purchase', 'state': 'open', 'currency_id': currency.id, 'active': True}
        ...
        asset_vals.update({
            ...
            'account_asset_id': account_1.id,
        })
        asset_1 = self.env['account.asset'].create(asset_vals)

        # Second Asset (from another company and after the first asset)
        asset_vals.update({
            'name': 'Asset 2',
            'company_id': company_2.id,
            'acquisition_date': asset_1.acquisition_date + relativedelta(days=1),
            'account_asset_id': account_2.id,
        })
        self.env['account.asset'].create(asset_vals)

        # Gross Increase of the First Asset (after the second asset)
        asset_vals.update({
            'name': 'Gross Increase',
            'original_value': 1,
            'company_id': company_1.id,
            'acquisition_date': asset_1.acquisition_date + relativedelta(days=2),
            'account_asset_id': account_1.id,
            'parent_id': asset_1.id,
        })
        self.env['account.asset'].create(asset_vals)

        report_options = {
            'multi_company': [{'id': company_1.id}, {'id': company_2.id}],
            'date': {
                'date_from': asset_1.acquisition_date,
                'date_to': asset_1.acquisition_date + relativedelta(days=3),
            },
        }
        report_lines = self.env['account.assets.report']._get_lines(report_options)
        line = [line for line in report_lines if line['id'].endswith(f'-{asset_1.id}')][0]
        book_value = line['columns'][-1].get('no_format_name')
        self.assertEqual(book_value, asset_1.book_value)
