    ...
    def action_post(self):
        ...
        if closing_move and "l10n_it_xml_export_monthly_tax_report_options" not in self.env.context:
            ...
            if last_posted_tax_closing:
                ...
                if closing_move.date.month - last_posted_tax_closing[0].date.month > 1:
                    raise UserError(_("You cannot post the tax closing of %(month)s without posting the previous month tax closing first.", month=closing_move.date.strftime("%m/%Y")))
            else:
                ...
                previous_move = self.env['account.move'].search_fetch(osv.expression.AND([
                    *self.env['account.move']._check_company_domain(self.company_id),
                    ...
                    ('date', '<', date_utils.start_of(closing_move.date, 'quarter' if quarterly else 'month')),
                    osv.expression.OR([
                        ('fiscal_position_id.country_id.code', '=', 'IT'),
                        osv.expression.AND([
                            ('fiscal_position_id', '=', False),
                ...
                if previous_move:
                    ...
                    while current <= closing_move.date.replace(day=1):
