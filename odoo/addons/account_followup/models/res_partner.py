...
class ResPartner(models.Model):
    ...
    def _compute_for_followup(self):
        ...
        """
        first_followup_level = self.env['account_followup.followup.line'].sudo().search([('company_id', '=', self.env.company.id)], order="delay asc", limit=1)
        ...
        for record in self:
            ...
            if record.id in followup_data:
                ...
                record.followup_level = self.env['account_followup.followup.line'].sudo().browse(followup_data[record.id]['followup_level']) or first_followup_level
            else:
