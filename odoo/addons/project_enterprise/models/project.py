...
class Task(models.Model):
    ...
    def create(self, vals_list):
        ...
        for task in tasks:
            if task.recurrence_id and task.planned_date_begin:
                task.recurrence_id._set_next_recurrence_date_from_start_date(task.planned_date_begin)
        return tasks
    ...
    def write(self, vals):
        ...
        # If we change the planned date begin and our values have to influence the recurrence,
        # we need to update the next recurrence date.
        recurrence_update = vals.get('recurrence_update', 'this')
        if recurrence_update != 'this':
            if vals.get('planned_date_begin'):
                for task in self:
                    task.recurrence_id._set_next_recurrence_date_from_start_date(vals['planned_date_begin'])
