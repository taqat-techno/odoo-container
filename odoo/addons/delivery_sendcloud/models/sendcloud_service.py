...
class SendCloud:
    ...
    def _send_shipment(self, picking, is_return=False):
        ...
        # unless the parcel is a return label
        if 'access_point_address' in picking.sale_id and picking.sale_id.access_point_address and not is_return:
    ...
    def _get_products_values(self, sale_order=None):
        ...
        for line in sale_order.order_line:
            if line.product_id.type not in ('consu', 'product') or line.display_type or float_is_zero(line.product_uom_qty, precision_rounding=line.product_uom.rounding) or line.product_uom_qty < 0:
                continue
