...
class AccountGeneralLedger(models.AbstractModel):
    ...
    def _get_saft_report_account_type(self, account):
        ...
        :return: False if no account type, otherwise a string with the account type"""
        return False
    ...
    def _fill_saft_report_general_ledger_values(self, options, values):
        ...
        for account, periods_results in accounts_results:
            ...
            res['account_vals_list'].append({
                'account': account,
                'type': self._get_saft_report_account_type(account),
        ...
        for line_vals in self._cr.dictfetchall():
            ...
            line_vals['is_credit'] = \
                line_vals['credit'] != 0 or \
                line_vals['amount_currency'] < 0 or \
                (
                    line_vals['debit'] == 0 and
                    line_vals['amount_currency'] == 0 and
                    line_vals['move_type'] in ('out_invoice', 'in_refund', 'out_receipt')
                )
    ...
    def _get_saft_report_template(self):
        """Meant to be overridden if using a custom template"""
        return self.env.ref('account_saft.saft_template')
    ...
    def get_xml(self, options):
        ...
        report_template = self._get_saft_report_template()
        content = report_template._render(template_vals)
