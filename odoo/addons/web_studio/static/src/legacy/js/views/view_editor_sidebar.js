...
const UNSUPPORTED_WIDGETS_FOR_X2M = [
    'selection',
    'selection_badge',
...
export const ViewEditorSidebar = Widget.extend(StandaloneFieldManagerMixin, {
    ...
    init: function (parent, params) {
        ...
        if (this.state.node && (this.state.node.tag === 'field' || this.state.node.tag === 'filter')) {
            ...
            const isEditingX2mListView = this.view_type === 'list' && this.isEditingX2m;
            let unsupportedWidgets = [
                UNSUPPORTED_WIDGETS_BY_VIEW[this.view_type] || [],
                isEditingX2mListView ? UNSUPPORTED_WIDGETS_FOR_X2M : [],
            ].flat();
    ...
    _getNewAttrsFromModifiers: function (modifiers) {
        ...
        _.each(modifiers, function (value, key) {
                ...
                if (keyInNodeAndAttrs && !isOriginNodeAttr && trueValue) { // modifier always applied, use modifier attribute
                    ...
                    if (keyInNodeAndAttrs) {
                        newAttributes[key] = "";
                    }
                    if (value !== false) {
                        attrs.push(_.str.sprintf("\"%s\": %s", key, Domain.prototype.arrayToString(value)));
                    }
                } else if (!keyInNodeAndAttrs && typeof value === "boolean") {
                    attrs.push(_.str.sprintf("\"%s\": %s", key, value ? "True" : "False"));
                }
        });
        ...
        if (attribute && $input.attr('type') !== 'file') {
            ...
            if (attribute.match('^rainbow')) {
                ...
                if (!_.contains(this.MODIFIERS_IN_NODE_AND_ATTRS, attribute)) {
                    ...
                    var newModifiers = _.extend({}, this.state.modifiers);
                    const changingInvisible = attribute === "invisible";
                    const isInList = this.view_type === "list";
                    if (changingInvisible && isInList && this.state.node && this.state.node.tag === "field") {
                        if ($input.is(':checked')) {
                            newModifiers["invisible"] = true;
                        } else {
                            newModifiers["column_invisible"] = false;
                            newModifiers["invisible"] = false;
                        }
                    } else {
