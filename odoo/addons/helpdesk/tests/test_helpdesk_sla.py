...
class HelpdeskSLA(SavepointCase):
    ...
    def test_deadlines_after_work(self):
        ...
        self.assertEqual(ticket.sla_deadline, fields.Datetime.now() + relativedelta(days=1, hour=11), "Day 0, 20h + 3h = Day 1, 8h + 3h = Day 1, 11h")

        self.sla.time_days = 1
        self.sla.time_hours = 3
        ticket = self.create_ticket(user_id=self.env.user.id)
        self._utils_set_create_date(ticket, fields.Datetime.now(), ticket)
        self.assertEqual(ticket.sla_deadline, fields.Datetime.now() + relativedelta(days=2, hour=11), "Day 0, 20h + 1d, 3h = Day 1, 8h + 1d, 3h = Day 2, 8h + 3h = Day 2, 11h")
    ...
    def test_deadlines_during_work(self):
        ...
        # on the ticket is the next day at 08:00
        self._utils_set_create_date(ticket, fields.Datetime.now(), ticket)
        self.assertEqual(ticket.sla_deadline, fields.Datetime.now() + relativedelta(days=0, hour=11), "Day 0, 8h + 3h = Day 0, 11h")
