...
registry.category("web_tour.tours").add("test_not_allowing_component_lot_creation", { test: true, steps: () => [
    {
        trigger: ".o_stock_barcode_main_menu",
        run: "scan TNACLC"
    },
...
registry.category("web_tour.tours").add("test_barcode_mo_creation_in_mo2", {
    steps: () => [
        ...
        { trigger: "input#product_id_0", run: "text Product4" },
        ...
        { trigger: ".o_menu_toggle", run: "click" },
        { trigger: ".o_app[data-menu-xmlid='stock_barcode.stock_barcode_menu']", run: "click"},
...
registry.category("web_tour.tours").add("test_select_mo_component_line_scan_package_type", {
    test: true, steps: () => [
        ...
        ...stepUtils.validateBarcodeOperation('.o_notification.border-danger'),
    ],
});

registry.category("web_tour.tours").add("test_create_all_transfers_for_3_step_manufacturing", {test: true, steps: () => [
    { trigger: ".o_kanban_card_header:contains('Manufacturing')", run: "click" },
    { trigger: ".o-kanban-button-new", run: "click" },
    { trigger: "button.o_add_line", run: "click" },
    { trigger: "input#product_id_0", run: "text Final" },
    { trigger: ".ui-autocomplete a:contains('Final Product')", run: "click" },
    { trigger: "div[name=product_id] .o_external_button", run(){} },
    { trigger: "button.o_save", run: "click" },
    { trigger: ".o_validate_page", run(){} },
]});
