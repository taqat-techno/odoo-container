...
class TestSubscription(TestSubscriptionCommon):
    ...
    def test_invoicing_access_rights(self):
        ...
        self.assertEqual(
            ...
            "We should get the amount invoiced for non-recurring products",
        )
    ...
    def test_credit_note_link(self):
        """ Test that credit note are still linked to the subscription
            when the credited product is deleted from the subscription
        """
        sub = self.env['sale.order'].create({
            'name': 'TestSubscription',
            'is_subscription': True,
        ...
        credit_note_wizard = self.env['account.move.reversal'].with_context(active_model="account.move", active_ids=inv.ids).create({
            'reason': 'Test credit note',
            'journal_id': inv.journal_id.id,
            'date': inv.date,
        })
        credit_note = self.env['account.move'].browse([credit_note_wizard.refund_moves()['res_id']])
        # crediting only the second product
        credit_note.line_ids.filtered(lambda r: r.product_id.id == self.product.id).unlink()

        # removing the 2nd product from the subscription
        sub.order_line.filtered(lambda r: r.product_id.id == self.product2.id).unlink()
        self.assertEqual(sub.invoice_ids, inv | credit_note)
    ...
    def test_subscription_discount_on_non_sub_lines(self):
        """
        Check that discounts manually set on non-subscription order lines are preserved in two cases:
        - when creating the subscription
        - after closing the subscription
        In this case, discounts for subscription lines should not be recomputed, so also check that.
        """
        DISCOUNT = 10
        pricelist_without_discount = self.company_data['default_pricelist'].copy({"discount_policy": "without_discount"})
        self.product2.recurring_invoice = False
        ...
        subscription = self.env['sale.order'].create({
            ...
            'partner_id': self.partner.id,
            'plan_id': self.plan_month.id,
            'pricelist_id': pricelist_without_discount.id,
            'order_line': [
                Command.create({
                    'name': 'Subscription product',
                    'product_id': self.product.id,
        ...
        self.assertEqual(subscription.order_line.mapped("discount"), [DISCOUNT, DISCOUNT])
