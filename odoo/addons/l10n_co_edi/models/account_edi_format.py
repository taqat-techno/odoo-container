    ...
    def _l10n_co_edi_prepare_tim_sections(self, taxes_dict, invoice_currency, retention, tax_details=None, actual_tax_details=None, in_COP=False):
        ...
        for grouping_key, tax_detail in actual_tax_details['tax_details'].items():
            ...
            if tax_type.code == '05':
                ...
                imp_2 = sum(vals['base_line_id'].product_id.volume * vals['base_line_id'].quantity
                            for vals in tax_detail['group_tax_details'])  # Volume
            else:
    ...
    def _l10n_co_edi_generate_xml(self, invoice):
        ...
        def group_tax_tim(tax_values):
            ...
            return {
                ...
                'l10n_co_edi_type': tax_values['tax_id'].l10n_co_edi_type,
            }

        def l10n_co_filter_to_apply(tax_values):
            return tax_values['tax_id'].l10n_co_edi_type.code not in code_to_filter

        tax_details_tim = invoice._prepare_edi_tax_details(filter_to_apply=l10n_co_filter_to_apply, grouping_key_generator=group_tax_tim, compute_mode='compute_all')
        tax_details = invoice._prepare_edi_tax_details(filter_to_apply=l10n_co_filter_to_apply, grouping_key_generator=group_tax_retention, compute_mode='compute_all')
        ...
        withholding_amount = '%.2f' % (invoice.amount_untaxed + sum(regular_tax_lines.mapped('price_total')))
