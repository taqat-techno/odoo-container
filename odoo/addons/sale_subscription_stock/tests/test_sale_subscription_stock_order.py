...
class TestSubscriptionStockOnOrder(TestSubscriptionStockCommon):
    ...
    def test_post_invoice_hook_exception_handler(self):
        ...
        with freeze_time("2024-10-01"):
            ...
            sub_stock_not_invoiced = self.env['sale.order'].create({
                'name': "Order With Delivery",
                'is_subscription': True,
                'partner_id': self.user_portal.partner_id.id,
                'plan_id': self.plan_month.id,
            ...
            (sub_stock_invoiced | sub_stock_not_invoiced | sub_service).action_confirm()
            self.env["sale.order"]._create_recurring_invoice()
            # Trigger the exception handler as no exception really happened
            (sub_stock_invoiced | sub_stock_not_invoiced | sub_service)._handle_post_invoice_hook_exception()
        ...
        # No subscription was not invoiced, no delivery needed to be created,
        # no activity should have been created
        self.assertEqual(len(sub_stock_not_invoiced.activity_ids), 0)
