...
class AnalyticLine(models.Model):
    ...
    def _apply_grid_grouped_expand(
        ...
        def read_row_fake_value(row_field, project, task):
            if row_field == 'project_id':
                return (project or task.project_id).sudo().name_get()[0]
    ...
    def check_if_allowed(self, vals=None, delete=False,):
        if not self.user_has_groups('hr_timesheet.group_timesheet_manager') and not self.env.su:
            ...
            subordinate_ids = self.env['hr.employee'].search([('id', 'child_of', self.env.user.employee_id.ids)])
            employees = self.env['hr.employee'].with_context(active_test=False).search([
                ...
                '|',
                    ...
                    ('timesheet_manager_id', 'in', [self._uid, False]),
                    ('id', 'in', subordinate_ids.ids),
            ])
    ...
    def adjust_grid(self, row_domain, column_field, column_value, cell_field, change):
        ...
        if line.project_id and not all(line.project_id.mapped("allow_timesheets")):
    ...
    def get_timer_data(self):
        favorite_project = self.env['project.project']
        last_timesheet_ids = self.search(self._get_favorite_project_id_domain(False), limit=5)
        if len(last_timesheet_ids.project_id) == 1:
            favorite_project = last_timesheet_ids.project_id
        return {
            'step_timer': int(self.env['ir.config_parameter'].sudo().get_param('timesheet_grid.timesheet_min_duration', 15)),
            'favorite_project': favorite_project.id,
            'favorite_project_name': favorite_project.sudo().name,
        }
    ...
    def _get_domain_for_validation_timesheets(self, validated=False):
        ...
        if not self.user_has_groups('hr_timesheet.group_timesheet_manager'):
            return expression.AND([domain, ['|', ('employee_id.timesheet_manager_id', 'in', [self.env.user.id, False]),
