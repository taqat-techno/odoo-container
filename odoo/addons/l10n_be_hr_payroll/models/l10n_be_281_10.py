                ...
                if payslip.date_from < date(2024, 4, 1):
                    ...
                    if round(abs(total_employment_bonus - (bonus_volet_A + bonus_volet_B)), 2) > 0.01:
                        if bonus_volet_A < total_employment_bonus and payslip.edited:
                            ...
                            raise UserError(_("Employment bonus: Volet A (%s €) + Volet B (%s €) not equal to total (%s) for payslip (id=%s) of employee %s", bonus_volet_A, bonus_volet_B, total_employment_bonus, payslip.id, payslip.employee_id.name))
                    if payslip.credit_note:
