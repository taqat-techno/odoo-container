...
Source: Opinion on the indexation of the amounts set in Article 1, paragraph 4, of the Royal Decree of 27 December 2004 implementing Articles 1409, § 1, paragraph 4, and 1409, § 1 bis, paragraph 4 , of the Judicial Code relating to the limitation of seizure when there are dependent children, MB, December 13, 2019.""")
    ...
    public_transport_reimbursed_amount = fields.Monetary(
        string='Public Transport Reimbursed amount', groups="hr_payroll.group_hr_payroll_user",
    ...
    l10n_be_group_insurance_rate = fields.Float(
        ...
        help="Should be between 0 and 100 %", groups="hr_payroll.group_hr_payroll_user")
    ...
    l10n_be_wage_with_mobility_budget = fields.Monetary(
        tracking=True, string="Wage with Mobility Budget", groups="hr_payroll.group_hr_payroll_user"
    )

    l10n_be_is_below_scale = fields.Boolean(
        string="Is below CP200 salary scale", compute='_compute_l10n_be_is_below_scale',
    ...
    def _check_dependent(self):
        ...
        for version in self:
            if (version.children < 0 or version.disabled_children_number < 0 or
               version.other_senior_dependent < 0 or version.other_disabled_senior_dependent < 0 or
               version.other_juniors_dependent < 0 or version.other_disabled_juniors_dependent < 0 or
               version.l10n_be_dependent_children_attachment < 0):
            ...
            if ((version.disabled_children_number > 0 and version.disabled_children_number > version.children) or
               (version.other_disabled_senior_dependent > 0 and version.other_disabled_senior_dependent > version.other_senior_dependent) or
               (version.other_disabled_juniors_dependent > 0 and version.other_disabled_juniors_dependent > version.other_juniors_dependent) or
               (version.l10n_be_dependent_children_attachment > 0 and version.l10n_be_dependent_children_attachment > version.children)):
    ...
    def _compute_dependent_children(self):
        for version in self:
            if version.disabled_children_bool:
                version.dependent_children = version.children + version.disabled_children_number
            else:
                version.dependent_children = version.children
    ...
    def _compute_l10n_be_is_below_scale(self):
        ...
        open_contracts = self.filtered(
            lambda c: (c.contract_date_start and c.contract_date_start <= fields.Date.today())
            and (not c.contract_date_end or c.contract_date_end >= fields.Date.today())
        ...
        for contract in open_contracts:
            company_seniority = relativedelta(fields.Date.today(), contract.contract_date_start).years
    ...
    def _search_l10n_be_is_below_scale(self, operator, value):
        ...
        below_contracts = self.env['hr.version'].search(
            [('state', 'in', ['draft', 'open'])]
    ...
    def _compute_ip_value(self):
        for version in self:
            version.ip_value = version.ip_wage_rate if version.ip else 0
    ...
    def _compute_meal_voucher_info(self):
        ...
        for version in self:
            version.meal_voucher_paid_by_employer = max(0, version.meal_voucher_amount - 1.09)
    ...
    def _compute_train_transport_reimbursed_amount(self):
        for version in self:
            version.train_transport_reimbursed_amount = version._get_train_transport_reimbursed_amount(version.train_transport_employee_amount)
    ...
    def _compute_public_transport_reimbursed_amount(self):
        for version in self:
            version.public_transport_reimbursed_amount = version._get_public_transport_reimbursed_amount(version.public_transport_employee_amount)

    @api.depends('employee_id')
    def _compute_employee_age(self):
        for version in self:
            if not version.employee_id or not version.employee_id.birthday:
                version.employee_age = 0
            else:
                version.employee_age = relativedelta(fields.Date.today(), version.employee_id.birthday).years
    ...
    def _compute_private_car_reimbursed_amount(self):
        for version in self:
            if version.transport_mode_private_car:
                amount = self._get_private_car_reimbursed_amount(version.km_home_work)
            else:
                amount = 0.0
            version.private_car_reimbursed_amount = amount
    ...
    def _compute_l10n_be_group_insurance_amount(self):
        for version in self:
            ...
            insurance_amount = version.wage * rate / 100.0
        ...
        for _date_start_day, work_entry_type, duration_sum in work_entries:
            ...
            if float_compare(duration_sum, number_of_hours_full_day, 2) != -1:
                if number_of_hours_full_day:
                    ...
                    # If not supposed to work in calendar attendances, then there are not time offs
                    number_of_days = 1
