...
class GeneralLedgerCustomHandler(models.AbstractModel):
    ...
    def _export_l10n_se_sie4_chart_of_account(self, options):
        ...
        accounts = self.env['account.account'].search(self.env['account.account']._check_company_domain(company_id))
    ...
    def _export_l10n_se_sie4_bs_balance(self, options):
        ...
        prev_ib_values = self._get_initial_balance_values(report, bs_accounts.mapped('id'), prev_year_options)
        curr_ib_values = self._get_initial_balance_values(report, bs_accounts.mapped('id'), options)
        next_ib_values = self._get_initial_balance_values(report, bs_accounts.mapped('id'), next_year_options)
    ...
    def _export_l10n_se_sie4_pl_balance(self, options):
        ...
        prev_account_sum_group = self.env['account.move.line']._read_group(prev_year_domain, ['account_id'], ['balance:sum'])
        curr_account_sum_group = self.env['account.move.line']._read_group(curr_year_domain, ['account_id'], ['balance:sum'])
