...
class IntrastatReportCustomHandler(models.AbstractModel):
    ...
    def _get_errors(self):
        return ['expired_trans', 'premature_trans', 'missing_trans', 'expired_comm', 'premature_comm', 'missing_comm', 'missing_unit', 'missing_weight']
    ...
    def _create_report_line(self, options, line_vals, line_id, number_values, warnings=None):
        ...
        for column in options['columns']:
            ...
            if expression_label == 'value_currency':
                invoice_currency_id = line_vals.get(column['column_group_key'], {}).get('invoice_currency_id')
                if invoice_currency_id == self.env.company.currency_id.id:
                    value = 0.0
                else:
                    currency = self.env['res.currency'].browse(invoice_currency_id)
            columns.append(report._build_column_dict(value, column, options=options, currency=currency))

        if warnings is not None:
            for column_group in options['column_groups']:
                for warning_code in self._get_errors():
    ...
    def _get_aml_line(self, report, parent_line_id, options, aml_data):
        ...
        for column in options['columns']:
            ...
            currency = col_expr_label == 'value_currency' and self.env['res.currency'].browse(aml_data['invoice_currency_id'])
            new_column = report._build_column_dict(col_value, column, options=options, currency=currency)
    ...
    def _prepare_query(self, options, column_group_key=None, expanded_line_options=None, offset=0, limit=None, order_by=True, query_params=None):
        # keeping _prepare_query() & _build_query() for stable policy reasons
        return self._build_query(options, column_group_key, expanded_line_options, offset, limit, order_by, query_params)

    def _build_query(self, options, column_group_key=None, expanded_line_options=None, offset=0, limit=None, order_by=True, query_params=None):
        ...
        query_params = {
            ...
            'missing_weights_warning': SQL("COALESCE(prod.weight, 0) = 0 AND code.supplementary_unit IS NULL AS missing_weight,"),
            'missing_trans_code_warning': SQL("transaction.id IS NULL AS missing_trans,"),
            **(query_params or {}),
        }
        ...
        if expanded_line_options:
            ...
            for key, value in where_values.items():
                where_cond.append(SQL(" AND {key} IS NOT DISTINCT FROM %s").format(key=Identifier(*key.split('.'))))
                where_params.append(value)
            where_cond.append(SQL(" AND COALESCE(inv_incoterm.code, comp_incoterm.code) IS NOT DISTINCT FROM %s"))
            where_params.append(format_where_params('incoterm_code'))
            where_cond.append(SQL(" AND COALESCE(inv_transport.code, comp_transport.code) IS NOT DISTINCT FROM %s"))
            where_params.append(format_where_params('transport_code'))
            expanded_line_options_where = SQL("\n").join(where_cond)
        else:
            expanded_line_options_where = SQL("")
        ...
        query = SQL("""(
            SELECT
                ...
                CASE WHEN (code.country_id IS NULL OR code.country_id = {country_id}) THEN {commodity_code} ELSE NULL END AS commodity_code,
                ...
                SIGN(account_move_line.quantity) * SIGN(account_move_line.price_unit) * ABS(account_move_line.amount_currency) AS value_currency,
                ...
                {missing_trans_code_warning}
                code.expiry_date <= account_move.invoice_date AS expired_{commodity_warning_suffix},
                code.start_date > account_move.invoice_date AS premature_{commodity_warning_suffix},
                code.id IS NULL as missing_{commodity_warning_suffix},
                COALESCE(prod.intrastat_supplementary_unit_amount, 0) = 0 AND code.supplementary_unit IS NOT NULL as missing_unit,
                {missing_weights_warning}
            ...
            WHERE
                ...
                {country_condition}
                {expanded_line_options_where}
                {intrastat_with_vat_where}
            {order_by}
            {tail}
            )
        """).format(
            # select
            user_lang=Literal(self.env.user.lang or get_lang(self.env).code),
            country_id=Literal(self.env.company.country_id.id),
            commodity_code=query_params['commodity_code'],
            missing_trans_code_warning=query_params['missing_trans_code_warning'],
            commodity_warning_suffix=query_params['commodity_warning_suffix'],
            missing_weights_warning=query_params['missing_weights_warning'],
            # from
            tables=tables,
            ...
            expanded_line_options_where=expanded_line_options_where,
            intrastat_with_vat_where=SQL("AND partner.vat IS NOT NULL" if options['intrastat_with_vat'] else ""),
            # order by
            order_by=SQL("ORDER BY invoice_date DESC, id" if order_by else ""),
            # tail
            tail=SQL(tail_query),
        )
    ...
    def _build_query_group(self, options, column_group_key=None, query_params=None):
        ...
        inner_query, params = self._prepare_query(options, column_group_key=column_group_key)
        query_params = {'commodity_warning_suffix': SQL("comm"), **(query_params or {})}
        query = SQL("""
          SELECT %s AS column_group_key,
                 ...
                 ARRAY_AGG(CASE WHEN intrastat_lines.expired_{commodity_warning_suffix} IS TRUE THEN intrastat_lines.product_id END) as expired_{commodity_warning_suffix},
                 ARRAY_AGG(CASE WHEN intrastat_lines.premature_{commodity_warning_suffix} IS TRUE THEN intrastat_lines.product_id END) as premature_{commodity_warning_suffix},
                 ARRAY_AGG(CASE WHEN intrastat_lines.missing_{commodity_warning_suffix} IS TRUE THEN intrastat_lines.product_id END) as missing_{commodity_warning_suffix},
                 ...
                 SUM(intrastat_lines.value_currency) as value_currency,
