...
class L10nCHEmployeeMonthlySnapshot(models.Model):
    ...
    def _compute_person(self):
        ...
        for snapshot in self:
            ...
            self._fill_xml_scheme(address["Address"], "Country", employee.with_context(lang='en_EN').private_country_id, lambda_f=lambda c: c.name.upper())
        ...
        for snapshot in self:
            ...
            if current_employee.l10n_ch_tax_code:
                ...
                if current_employee._get_l10n_ch_declaration_marital() in ["married", "registeredPartnership"]:
                    ...
                    self._fill_xml_scheme(marriage_partner_address, "Country", current_employee.with_context(lang='en_EN').l10n_ch_spouse_country_id, lambda_f=lambda c: c.name.upper())
