...
class L10nCHSwissdecJobResult(models.Model):
            ...
            for tax_salary in tax_salaries:
                ...
                for corr in corrections:
                    if corr.get("Reversal") is not None:
                        ...
                        message_log.append(_("Source Tax Correction (%s) imported for %s", correction._get_html_link(), employee._get_html_link()))
                    ...
                    if corr.get("AwaitCorrectionFromCompany") is not None:
                        ...
                        message_log.append(Markup(_("Source Tax Correction (%s) imported for %s", correction._get_html_link(), employee._get_html_link())))
        if message_log:
            ...
            for i, person in enumerate(persons_with_poi):
                ...
                if len(employee) == 0:
                    ...
                    employee_declaration_vals.append({
                        ...
                        'res_model': self._name,
                        'res_id': self.id,
                        'pdf_filename': _("Proof_of_insurance_%s_%s", self.transmission_date.year, employee.name),
    ...
    def action_open_proof_of_insurance(self):
        self.ensure_one()
        return {
            'name': _('Proof of Insurances %s-%s', self.transmission_date.year, str(self.transmission_date.month).zfill(2)),
