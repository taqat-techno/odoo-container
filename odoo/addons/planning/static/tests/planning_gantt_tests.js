...
QUnit.module("Views", (hooks) => {
    ...
    QUnit.test(
        "Verify Hours in Planning Dialog When Clicking on cell for Off Days and Working Days in Gantt View",
        async function (assert) {
            patchDate(2024, 9, 19, 0, 0, 0);
            ...
            await makeView({
                ...
                resModel: "task",
                serverData,
                arch: '<gantt js_class="planning_gantt" default_scale="week" date_start="start_datetime" date_stop="end_datetime" plan="0" display_unavailability="1"/>',
            ...
            assert.strictEqual(
                target.querySelector(".o_field_widget[name=start_datetime] .o_input").value,
                "10/18/2024 00:00:00",
                "The start date should be the minimum time for the selected date."
            );
            assert.strictEqual(
                target.querySelector(".o_field_widget[name=end_datetime] .o_input").value,
                "10/18/2024 23:59:59",
                "The end date should be the maximum time for the selected date."
            );
    ...
    QUnit.test(
        "Verify Hours in Planning Dialog When Clicking 'New' Button for Off Days in Gantt View",
        async function (assert) {
            patchDate(2024, 10, 19, 0, 0, 0);
            const unavailabilities = [
                {
                    start: "2024-11-17 10:00:00",
                    stop: "2024-11-20 14:00:00",
                },
            ];
            ...
            await makeView({
                ...
                resModel: "task",
                serverData,
                arch: '<gantt js_class="planning_gantt" default_scale="week" date_start="start_datetime" date_stop="end_datetime" display_unavailability="1"/>',
                async mockRPC(_, { method, args }) {
                    if (method === "gantt_unavailability") {
                        const rows = args[4];
                        for (const r of rows) {
