...
QUnit.module("Planning.planning_calendar_tests", ({ beforeEach }) => {
    ...
    QUnit.test(
        "Verify Hours in Planning Dialog When Clicking on Off Days and Working Days Calendar view",
        async function (assert) {
            ...
            await makeView({
                ...
                async mockRPC(_, { method, args }) {
                    if (method === "get_unusual_days") {
                        return { "2021-06-20": true, "2021-06-26": true };
                    }
                },
            });

            // click on dayoff day
            await clickDate(target, "2021-06-20");
            assert.containsOnce(target, ".o_dialog");
            // check that the dialog is opened with prefilled fields
            assert.strictEqual(
                target.querySelector(".o_field_widget[name=start] .o_input").value,
                "06/20/2021 00:00:00",
                "The start date should be the minimum time for the selected date."
            );
            assert.strictEqual(
                target.querySelector(".o_field_widget[name=stop] .o_input").value,
                "06/20/2021 23:59:59",
            ...
            await clickDate(target, "2021-06-22");
            assert.containsOnce(target, ".o_dialog");
            // check that the dialog is opened with prefilled fields
            assert.strictEqual(
                target.querySelector(".o_field_widget[name=start] .o_input").value,
