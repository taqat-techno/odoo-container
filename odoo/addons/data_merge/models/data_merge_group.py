    ...
    def _log_merge(self, master_record, merged_records, chatter_data):
        ...
        for rec in merged_records:
            ...
            if self.model_id.removal_mode == 'archive':
                rec._original_records().message_post_with_view('data_merge.merge_message', values=values, subtype_id=self.env.ref('mail.mt_note').id)
            master_record._original_records().message_post_with_view('data_merge.merge_message_master', values=master_values, subtype_id=self.env.ref('mail.mt_note').id)
    ...
    def _cleanup(self):
        ...
        for group in self:
            ...
            for rec in group.record_ids:
                if rec.res_id not in original_records:
