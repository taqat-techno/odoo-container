...
class SpanishMod349TaxReportCustomHandler(models.AbstractModel):
    ...
    def _custom_modelo349_common(self, options, current_groupby, key=None, move_type=None):
                    ...
                    if res_line['move_type'] in ('in_refund', 'out_refund') and res_line['reversed_entry_id']:
                        ...
                        if move_type == 'invoice' and options['date']['date_from'] <= matching_move.date.strftime('%Y-%m-%d') <= options['date']['date_to']:
                            ...
                            if matching_move.id not in treated_moves:
                                ...
                                result_dict['value'] += matching_move.amount_total - sum(refund.amount_total for refund in matching_move.reversal_move_id)
        ...
        query = SQL("""
            SELECT
                DISTINCT account_move.id AS move_id,
    ...
    def _write_type2_refund_records(self, options, report_data, current_company, mod_349_type, invoice_report_line_xml_id, report_period, report_year):
        ...
        for refund_invoice in self.env['account.move'].search([
            ...
            if original_invoice.id not in matched_moves:
                ...
                period_dict[f"{invoice_period}{invoice_year}"]['new_balance'] += original_invoice.amount_total - sum(refund.amount_total for refund in original_invoice.reversal_move_id)
