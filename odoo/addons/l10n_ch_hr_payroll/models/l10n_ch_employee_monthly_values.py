...
class L10nCHEmployeeMonthlySnapshot(models.Model):
    ...
    _check_month = models.Constraint(
    ...
    def _compute_employee_meta_data(self):
        ...
        for snapshot in self:
            version = snapshot.employee_id._get_version(date=datetime.date(year=snapshot.year, month=snapshot.month, day=1))
            meta_data = {
                "st-code": version.l10n_ch_tax_code or "",
                "st-canton": version.l10n_ch_source_tax_canton or "",
                "st-municipality": version.l10n_ch_source_tax_municipality or "",
                "children_deduction": version.children,
                "st-type": version.l10n_ch_tax_scale_type,
                "aci_approved": "GrantTaxAtSourceCode" if version.l10n_ch_tax_specially_approved else "",
                "txb-code": f"{version.private_country_id.code}-{version.l10n_ch_source_tax_canton}" if version.l10n_ch_cross_border_commuter and version.l10n_ch_source_tax_canton else "",
                "lpp_institution": swissdec_helper.get_institution_id_ref(version.l10n_ch_lpp_insurance_id) if version.l10n_ch_lpp_insurance_id and not version.l10n_ch_lpp_not_insured else "",
                "lpp_codes": sorted(version.l10n_ch_lpp_solutions.mapped('code')),
    ...
    def _compute_person(self):
        mapped_contracts = dict(self.env["l10n.ch.occupation"]._read_group(domain=[
        ...
        for snapshot in self:
            employee = snapshot.employee_id._get_version(date=datetime.date(year=snapshot.year, month=snapshot.month, day=1))
            ...
            social_insurance_identification = {
                "Social-InsuranceIdentification": {
                    "SV-AS-Number": employee.employee_id.l10n_ch_sv_as_number
                } if employee.employee_id.l10n_ch_sv_as_number else {
                    "unknown": XSD_SKIP_VALUE
                }
            }
            employee_contracts = mapped_contracts.get(snapshot.employee_id, self.env['l10n.ch.occupation'])
            ...
            if valid_contracts_this_month:
                ...
                if employee.irregular_working_time:
                    working_time = {
                    ...
                    working_time = {}
                    hours_dict = {}
                    self._fill_xml_scheme(hours_dict, "WeeklyHours", employee.l10n_ch_weekly_hours, lambda_f=self._amount2str)
                    self._fill_xml_scheme(hours_dict, "WeeklyLessons", employee.l10n_ch_weekly_lessons, lambda_f=self._amount2str)
                    if employee.l10n_ch_weekly_hours and employee.l10n_ch_weekly_lessons and not employee.irregular_working_time:
                        working_time["Steady"] = {
                    ...
                    working_time["Steady"]["ActivityRate"] = self._amount2str(employee.l10n_ch_current_occupation_rate)
    ...
    def _compute_additional_particular(self):
        for snapshot in self:
            ...
            if current_employee.l10n_ch_tax_code:
                ...
                children = []
                for child in current_employee.employee_id.l10n_ch_children:
                    child_info = dict()
    ...
    def _compute_monthly_statistics(self):
        ...
        for snapshot in self:
            ...
            if relevant_monthly_payslips:
                ...
                relevant_contract = last_monthly_payslip.version_id
                statistic_salary = {
                ...
                self._fill_xml_scheme(additional_particular, "Education", relevant_contract.employee_id.certificate, True, lambda_f=lambda c: c if c in ALLOWED_EDUCATION else "mandatorySchoolOnly")
                ...
                self._fill_xml_scheme(additional_particular, "PermanentStaffPublicAdmin", relevant_contract.l10n_ch_permanent_staff_public_admin, condition=False, lambda_f=lambda v: XSD_SKIP_VALUE if v else False)
                self._fill_xml_scheme(additional_particular, "TemporaryAgencyWorker", relevant_contract.l10n_ch_interim_worker, condition=False, lambda_f=lambda v: XSD_SKIP_VALUE if v else False)
                self._fill_xml_scheme(additional_particular, "FlexProfiling", relevant_contract.l10n_ch_flex_profiling, condition=False)
                statistic_salary["AdditionalParticulars"] = additional_particular
                kind_of_wage_payment = dict()
                working_time = swissdec_declaration.get_workplace_working_hours(relevant_contract.l10n_ch_location_unit_id)
                contractual_13_14 = []
                ...
                if relevant_contract.wage_type == "monthly":
                    ...
                    self._fill_xml_scheme(kind_of_wage_payment["NoTimeConstraint"], "Vacation", relevant_contract.l10n_ch_contractual_holidays_rate, False, lambda_f=self._amount2str)
                    self._fill_xml_scheme(kind_of_wage_payment["NoTimeConstraint"], "PublicHolidayCompensation", relevant_contract.l10n_ch_contractual_public_holidays_rate, False, lambda_f=self._amount2str)
    ...
    def _compute_mutations(self):
        ...
        self.is_mutations.unlink()
        self.lpp_mutations.unlink()
        ...
        for snapshot in self:
            ...
            if valid_source_tax_snapshot and snapshot.employee_meta_data:
                ...
                entries = mapped_contracts.get(snapshot.employee_id, self.env['l10n.ch.occupation']).filtered(lambda c: c.date_start.month == snapshot.month and c.date_start.year == snapshot.year).mapped("date_start")
                withdrawals = mapped_contracts.get(snapshot.employee_id, self.env['l10n.ch.occupation']).filtered(lambda c: c.date_end and c.date_end.month == snapshot.month and c.date_end.year == snapshot.year).mapped("date_end")
                ...
                if entries or withdrawals:
                    ...
                    if snapshot.employee_id.l10n_ch_has_withholding_tax:
                        ...
                        for mutation in st_mutations:
                            if mutation.startswith(('withdrawalCanton', 'entryCanton')) and ('withdrawalNat' in st_mutations or 'withdrawalSettled' in st_mutations):
                                continue
                    ...
                    for mutation in lpp_mutation:
                        lpp_vals += [(0, 0, {
                            "employee_id": snapshot.employee_id.id,
                            "version_id": snapshot.employee_id.version_id.id,
                            "reason": mutation,
