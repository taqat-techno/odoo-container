...
class L10nCHEmployeeYearlySnapshot(models.Model):
    ...
    _check_unique = models.Constraint(
        ...
        "Yearly values for this employee already exists for the year.",
    )
        ...
        for snapshot_y in yearly_values:
            ...
            if any(current_is_log_lines.mapped('source_tax_canton')):
                ...
                if not corrections_present and len(set(all_cantons)) == 1:
                    _year_sub = 1 if snapshot.month == 1 else 0
    ...
    def _get_monthly_ema(self, year, month, company_id):
        ...
        for emp, date_y, date_m, slip in paid_slips:
            if slip:
                mapped_payslips[emp][date_y.year][date_m.month] = slip[-1]

        first_day_of_month = datetime.date(year, month, 1)
        last_day_of_month = first_day_of_month + relativedelta(months=1, days=-1)

        mapped_contracts = dict(self.env["l10n.ch.occupation"]._read_group(
            domain=[
        ...
        for snapshot_y in yearly_values:
            ...
            snapshot_contracts = mapped_contracts.get(snapshot.employee_id, self.env['l10n.ch.occupation']).filtered(lambda c: (c.date_start <= last_day_of_month and (c.date_end >= first_day_of_month if c.date_end else True)))
            ema_values = dict()
            avs_ahv_salaries = []
            fak_caf_salaries = []
            bvg_lpp_salaries = []
            ...
            for c in snapshot_contracts:
                version_date_start = snapshot.employee_id._get_version(c.date_start)
                version_date_end = snapshot.employee_id._get_version(c.date_end or last_day_of_month)
                ...
                caf_canton_change = (previous_payslip.l10n_ch_entry, previous_payslip.l10n_ch_withdrawal) == (current_payslip.l10n_ch_entry, current_payslip.l10n_ch_withdrawal) and (previous_payslip.l10n_ch_entry, previous_payslip.l10n_ch_withdrawal) == (c.date_start, c.date_end) and previous_payslip.l10n_ch_location_unit_id.canton != current_payslip.l10n_ch_location_unit_id.canton
                ...
                lpp_entry_this_month = version_date_start.l10n_ch_lpp_insurance_id and version_date_start.l10n_ch_lpp_entry_valid_as_of and version_date_start.l10n_ch_lpp_entry_valid_as_of.month == snapshot.month and version_date_start.l10n_ch_lpp_entry_valid_as_of.year == snapshot.year
                lpp_withdrawal_this_month = version_date_start.l10n_ch_lpp_insurance_id and version_date_start.l10n_ch_lpp_withdrawal_valid_as_of and version_date_start.l10n_ch_lpp_withdrawal_valid_as_of.month == snapshot.month and version_date_start.l10n_ch_lpp_withdrawal_valid_as_of.year == snapshot.year

                if contract_start_this_month and version_date_start.l10n_ch_avs_status != "youth":
                    avs_ahv_salaries.append(swissdec_declaration.create_ahv_avs_ema(
                        institution_id=version_date_start.l10n_ch_social_insurance_id,
                    ...
                    global_avs_institutions += version_date_start.l10n_ch_social_insurance_id
                ...
                if contract_end_this_month and version_date_start.l10n_ch_avs_status != "youth":
                    avs_ahv_salaries.append(swissdec_declaration.create_ahv_avs_ema(
                        institution_id=version_date_end.l10n_ch_social_insurance_id,
                    ...
                    global_avs_institutions += version_date_end.l10n_ch_social_insurance_id

                if contract_start_this_month and version_date_start.l10n_ch_avs_status != "youth":
                    fak_caf_salaries.append(swissdec_declaration.create_fak_caf_ema(
                        institution_id=version_date_start.l10n_ch_compensation_fund_id,
                        canton=version_date_start.l10n_ch_location_unit_id.canton,
                        declaration_category="Entry",
                        valid_as_of=format_date(self.env, c.date_start, date_format='yyyy-MM-dd'),
                        reason="entryCompany",
                    ...
                    global_caf_institutions += version_date_start.l10n_ch_compensation_fund_id
                ...
                if contract_end_this_month and version_date_start.l10n_ch_avs_status != "youth":
                    fak_caf_salaries.append(swissdec_declaration.create_fak_caf_ema(
                        institution_id=version_date_end.l10n_ch_compensation_fund_id,
                        canton=version_date_end.l10n_ch_location_unit_id.canton,
                        declaration_category="Withdrawal",
                        valid_as_of=format_date(self.env, c.date_end, date_format='yyyy-MM-dd'),
                    ...
                    global_caf_institutions += version_date_end.l10n_ch_compensation_fund_id

                if caf_canton_change and version_date_start.l10n_ch_avs_status != "youth":
                    fak_caf_salaries.append(swissdec_declaration.create_fak_caf_ema(
                ...
                if lpp_entry_this_month and version_date_start.l10n_ch_avs_status != "youth":
                    lpp_declarations["Entry"].append(
                        {"ValidAsOf": format_date(self.env, version_date_start.l10n_ch_lpp_entry_valid_as_of, date_format='yyyy-MM-dd'), "Reason":  version_date_start.l10n_ch_lpp_entry_reason}
                    )
                if lpp_withdrawal_this_month and version_date_end.l10n_ch_avs_status != "youth":
                    lpp_declarations["Withdrawal"].append(
                        {"ValidAsOf": format_date(self.env, version_date_end.l10n_ch_lpp_withdrawal_valid_as_of, date_format='yyyy-MM-dd'), "Reason": version_date_end.l10n_ch_lpp_withdrawal_reason}
                    )
                if (not version_date_start.l10n_ch_lpp_not_insured) and version_date_start.l10n_ch_lpp_insurance_id and version_date_start.l10n_ch_avs_status != "youth":
                ...
                if any(lpp_declarations.values()):
                    bvg_lpp_salaries.append(swissdec_declaration.create_bvg_lpp_ema(
                        institution_id=version_date_start.l10n_ch_lpp_insurance_id,
                        declarations=lpp_declarations,
                        codes=version_date_start.l10n_ch_lpp_solutions.mapped('code') if version_date_start.l10n_ch_lpp_solutions else False,
                        bvg_lpp_annual_basis=snapshot.bvg_lpp_annual_basis
                    ))
                    global_lpp_institutions += version_date_start.l10n_ch_lpp_insurance_id
        ...
        for section in rules_grouped_by_certificate_section:
            if section in CERTIFICATE_RULE_MAPPING:
                ...
                if section in ['2.3', '3', '4', '7', '13.1.2', '13.2.3']:
                    ...
                    text = ', '.join(sorted(set(payslip_lines.mapped('name'))))
    ...
    def _get_yearly_retrospective(self, year, month, company_id, incomplete_declaration=False):
                ...
                for m, payslips in months.items():
                    if m <= month or y < year:
                        for payslip in payslips.filtered(lambda p: p.l10n_ch_txb_code and p.l10n_ch_is_code and not p.l10n_ch_after_departure_payment):
                            applicable_date_start = max(payslip.date_from, payslip.l10n_ch_entry)
                            applicable_date_start_s = format_date(self.env, applicable_date_start, date_format='yyyy-MM-dd')
                            applicable_date_end = min(payslip.date_to, payslip.l10n_ch_withdrawal if payslip.l10n_ch_withdrawal else payslip.date_to)
        ...
        for snapshot in yearly_values:
            ...
            # AVS Domain
            if relevant_slips:
                slips_grouped_by_contract = relevant_slips.grouped(lambda p: (p.l10n_ch_entry, p.l10n_ch_withdrawal))
                for contract, slips_c in slips_grouped_by_contract.items():
                    ...
                    for institution, slips_i in slips_grouped_by_institution.items():
                        ...
                        for avs_status, slips_status in slips_grouped_by_avs_status.items():
                            start_avs = max(
                                datetime.date(snapshot.year - year_delta, 1, 1),
                                contract[0],
                                min(slips_status.filtered(lambda p: not p.l10n_ch_after_departure_payment).mapped('date_from')))

                            if contract[1]:
                                end_avs = min(contract[1], max(slips_status.filtered(lambda p: not p.l10n_ch_after_departure_payment).mapped('date_to')))
                            else:
                    ...
                    for code in laa_group_slices:
                        for code_period_slips in laa_group_slices[code]:
                            ...
                            if contract[1]:
                                end_laa = min(contract[1], max(code_period_slips.filtered(lambda p: not p.l10n_ch_after_departure_payment).mapped('date_to')))
                            else:
                    ...
                    for laac_code in laac_group_slices:
                        for laac_code_period_slips in laac_group_slices[laac_code]:
                            ...
                            if contract[1]:
                                end_laac = min(contract[1], max([p for p in laac_code_period_slips if not p[1].l10n_ch_after_departure_payment], key=lambda p: p[1].date_to)[1].date_to)
                            else:
                    ...
                    for ijm_code in ijm_group_slices:
                        for ijm_code_period_slips in ijm_group_slices[ijm_code]:
                            ...
                            if contract[1]:
                                end_ijm = min(contract[1], max([p for p in ijm_code_period_slips if not p[1].l10n_ch_after_departure_payment], key=lambda p: p[1].date_to)[1].date_to)
                            else:
                    ...
                    for caf in slips_grouped_by_caf:
                        ...
                        for canton in slips_grouped_by_work_canton:
                            ...
                            if contract[1]:
                                end_caf = min(contract[1], max(slips_grouped_by_work_canton[canton].filtered(lambda p: not p.l10n_ch_after_departure_payment).mapped('date_to')))
                            else:
                    ...
                    for contract in slips_grouped_by_contract:
                        ...
                        for st_c in txb_slips:
                            for slip_period in txb_slips[st_c]:
                                ...
                                if contract[1]:
                                    end_txb = min(contract[1], max(slip_period.filtered(lambda p: not p.l10n_ch_after_departure_payment).mapped('date_to')))
                                else:
                ...
                for i, current_certificate in enumerate(all_profiles):
                    ...
                    if relevant_tax_slips:
                        ...
                        period_from = max(first_slip.l10n_ch_entry, datetime.date(snapshot.year - year_delta, 1, 1))
                        period_to = min(last_slip.date_to, last_slip.l10n_ch_withdrawal or datetime.date(snapshot.year - year_delta, 12, 31))
    ...
    def _get_salary_rectificates(self, year, month, company_id, original_date, to_replace):
        ...
        for snapshot in yearly_values.filtered(lambda s: s.employee_id.registration_number in to_replace):
            ...
            for i, current_certificate in enumerate(all_profiles):
                ...
                period_to = min(last_slip.date_to,
                                last_slip.l10n_ch_withdrawal or datetime.date(snapshot.year - year_delta, 12, 31))
                period_from = max(period_from, current_certificate.valid_from)
                period_to = min(period_to, next_profile_start)
        ...
        if staff:
            ...
            _institutions_to_process = [SwissdecInstitution("Tax")]
            declaration = {
                **swissdec_declaration.get_company_description(company_id),
                **staff_declaration,
                "SalaryCounters": swissdec_declaration.get_salary_tag_counter(staff_declaration),
