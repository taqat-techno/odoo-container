...
class TestSalePlanning(TestCommonSalePlanning):
    ...
    def test_recurrent_shift_date_change(self):
        ...
        1. Create a recurring shift that repeats 3 times monthly.
        2. Access the second slot from the generated recurrence.
        3. Modify the second slot's start and end datetime, and set 'recurrence_update': 'all'.
        4. Confirm that no MissingError is raised.
        """
        ...
        second_slot = first_slot.recurrency_id.slot_ids[1]
        try:
            second_slot.write({
                'start_datetime': first_slot_dt + timedelta(days=3, hours=8),
                'end_datetime': first_slot_dt + timedelta(days=3, hours=9),
                'recurrence_update': 'all',
            })
        except MissingError:
            self.fail("raised MissingError due to unfiltered deleted slot")
