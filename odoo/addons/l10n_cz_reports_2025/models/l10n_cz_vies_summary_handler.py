...
class CzechVIESSummaryReportCustomHandler(models.AbstractModel):
    ...
    def export_to_xml(self, options):
        ...
        for report_line in report_lines:
            ...
            if markup != 'groupby:l10n_cz_transaction_code':
                continue
        ...
        query, params = SQL(
            """
                SELECT %(select_from_groupby)s
                    ...
                    SUM(account_move_line.quantity * (CASE WHEN am.move_type IN ('out_refund', 'in_refund') THEN -1 ELSE 1 END)) AS supplies_number,
                    SUM(ABS(account_move_line.balance) * (CASE WHEN am.move_type IN ('out_refund', 'in_refund') THEN -1 ELSE 1 END))  AS total_value
                FROM %(tables)s
