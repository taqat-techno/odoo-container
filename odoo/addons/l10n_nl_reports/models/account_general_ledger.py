...
class ReportAccountGeneralLedger(models.AbstractModel):
    ...
    def _l10n_nl_reports_get_xaf_v2(self, options, company, date_from, date_to, periods):
        ...
        def get_vals_dict(iso_country_codes):
                ...
                self.env.cr.execute(f"""
                    ...
                    SELECT * FROM (
                        SELECT journal.id AS journal_id,
                           ...
                           move.amount_total AS move_amount,
                           move.move_type IN ('out_invoice', 'out_refund', 'in_refund', 'in_invoice', 'out_receipt', 'in_receipt') AS move_is_invoice,
            ...
            check_forbidden_countries(res_list, iso_country_codes)
        ...
        with tools.file_open('l10n_nl_reports/data/xml_audit_file_3_2.xsd', 'rb') as xsd:
            country_code_container = etree.fromstring(xsd.read()).find(
            ...
            vals_dict = get_vals_dict(iso_country_codes)
            values = {
                **self._get_opening_balance_vals(options),
                'company': company,
