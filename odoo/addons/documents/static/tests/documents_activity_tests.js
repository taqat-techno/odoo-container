...
QUnit.module("documents", {}, function () {
    QUnit.module(
        ...
        function () {
            ...
            QUnit.test(
                "documents activity: domain is preserved after archiving",
                async function (assert) {
                    let reloadDomain;

                    function onRouteCalled(args) {
                        if (args.method === "web_search_read" && args.model === "documents.document") {
                            // Capture the domain passed during the reload
                            reloadDomain = args.kwargs.domain;
                        }
                        if (args.method === "action_archive" && args.model === "documents.document") {
                            assert.step("action_archive");
                        }
                    ...
                    const filteredDomain = [["name", "=", "Document 1"]];
                    ...
                    // Document 1 is visible due to domain filter
                    const visibleNames = [...target.querySelectorAll(".o_activity_record")].map(
                        (el) => el.innerText
                    );
                    assert.deepEqual(visibleNames, ["Document 1"], "only filtered document is visible");

                    // Archive Document 1
                    await click(".o_data_row .o_activity_record");
                    await nextTick();
                    await click(".o_documents_inspector .fa-trash");
                    await nextTick();
                    const confirmButton = document.querySelector(".modal button:first-child");
                    await confirmButton.click();
                    await nextTick();

                    // The view should reload with the same domain used initially
                    assert.deepEqual(reloadDomain, [...filteredDomain, ["activity_ids.active", "in", [true, false]]], "domain is preserved after archive");
                    assert.verifySteps(["action_archive"]);
                }
            );
        }
