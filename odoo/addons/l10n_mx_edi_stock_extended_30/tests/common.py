...
class TestMXDeliveryGuideCommon(TestMxEdiCommon):
    ...
    def setUpClass(cls):
        ...
        cls.partner_a = cls.env['res.partner'].create({
            ...
            'street': 'Street Calle',
            'city_id': cls.env.ref('l10n_mx_edi.res_city_mx_chh_032').id,
    ...
    def create_picking(self, partner_id=None, picking_type_id=None):
        picking = self.env['stock.picking'].create({
            'location_id': self.new_wh.lot_stock_id.id,
            'location_dest_id': self.customer_location.id,
            'picking_type_id': picking_type_id or self.new_wh.out_type_id.id,
        ...
        self.env['stock.move'].create({
            ...
            'location_dest_id': self.customer_location.id,
            'state': 'confirmed',
            'description_picking': self.productA.name,
        })

        self.env['stock.quant']._update_available_quantity(self.productA, self.new_wh.lot_stock_id, 10.0)
        picking.action_assign()
        picking.move_lines[0].move_line_ids[0].qty_done = 10
