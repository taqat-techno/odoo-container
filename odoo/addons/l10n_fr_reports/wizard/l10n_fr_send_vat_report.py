...
class L10nFrSendVatReport(models.TransientModel):
    ...
    def _decompose_l10_fr_options(self, options):
        dt_from = fields.Date.to_date(options['date']['date_from'])
        dt_to = fields.Date.to_date(options['date']['date_to'])
        report = self.env['account.report'].browse(options['report_id'])
        lines = report._get_lines(options)
        return dt_from, dt_to, report, lines

    def _generate_new_l10n_fr_options(self):
        ...
        options = report.get_options({'no_format': True, 'date': {'date_from': dt_from, 'date_to': dt_to}, 'filter_unfold_all': True})
        return options
    ...
    def _get_formatted_edi_values(self, report, lines):
        ...
        for line in lines:
            ...
            if model == 'account.report.line':
                ...
                if edi_id := CODE_TO_EDI_ID.get(report_line.code):
    ...
    def _prepare_edi_vals(self, options):
        ...
        form_vals = self._get_formatted_edi_values(report, lines)
        if not form_vals:
    ...
    def send_vat_return(self):
        ...
        vat_report_name = self._get_vat_report_name(dt_from, dt_to)
        self._send_xml_to_aspone(xml_content, vat_report_name, dt_from, dt_to)

    def _send_xml_to_aspone(self, xml_content, export_name, dt_from, dt_to):
