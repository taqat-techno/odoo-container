...
registry.category("web_tour.tours").add("test_barcode_batch_receipt_1", {
    steps: () => [
        ...
        {
            trigger:
                '.o_barcode_line.o_highlight[data-barcode="productserial1"]:contains("SN-LHOOQ")',
            run: function () {
                currentViewState.scanMessage = "scan_serial";
                checkState(currentViewState);
            },
        ...
        {
            trigger:
                '.o_sublines .o_barcode_line.o_highlight:contains("lot0002") .qty-done:contains("4")',
            run: function () {
                const sublines = helper.getSublines({ barcode: "productlot1" });
        ...
        {
            trigger:
                '.o_barcode_line.o_highlight:contains("lot0002"):contains("picking_receipt_3") .qty-done:contains("4")',
            run: function () {
        ...
        {
            trigger:
                '.o_sublines .o_barcode_line.o_highlight:contains("lot0002") .qty-done:contains("5")',
            run: function () {
                const sublines = helper.getSublines({ barcode: "productlot1" });
                helper.assert(sublines.length, 2, "Expect 2 sublines for productlot1");
                helper.assertLineQty(sublines[0], "4");
    ...
    steps: () => [
        {
            ...
            run: function () {
                ...
                helper.assertLinesBelongTo(
                    [lines[1], lines[2], lines[6], lines[7]],
                    "picking_delivery_2"
                );
        ...
        {
            trigger:
                ".o_notification:has(.o_notification_bar.bg-danger):contains('Scan the current location to confirm that.')",
            run: "scan LOC-01-02-00",
        },
        {
        ...
        {
            trigger:
                ".o_barcode_line[data-barcode='product2']:nth-child(3).o_selected.o_line_completed",
            run: function () {
        ...
        {
            trigger:
                ".o_barcode_location_line[data-location='WH/Stock/Section 4']+.o_barcode_line.o_selected.o_line_completed",
            run: "scan shelf5",
        },
...
registry.category("web_tour.tours").add("test_barcode_batch_scan_lots", {
    steps: () => [
        ...
        // Open the batch receipt by scanning it.
        {
            ...
            run: "scan test_barcode_batch_scan_lots - receipt",
        },
        ...
        {
            trigger: ".o_stock_barcode_main_menu",
            run: "scan test_barcode_batch_scan_lots - delivery",
        },
        {
    ...
    steps: () => [
        ...
        // from here should be the same as test_barcode_batch_delivery_1 => just check that it initially looks the same
        {
            ...
            run: function () {
                ...
                helper.assertLinesBelongTo(
                    [lines[1], lines[2], lines[5], lines[6]],
                    "picking_delivery_2"
                );
            },
        },
