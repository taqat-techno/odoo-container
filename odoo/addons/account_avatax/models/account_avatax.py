...
class AccountAvatax(models.AbstractModel):
    ...
    is_avatax = fields.Boolean(compute='_compute_is_avatax', help='Technical field used for the visibility of fields and buttons.')
        ...
        def find_or_create_tax(doc, detail):
            def repartition_line(repartition_type, account=None):
                return (0, 0, {
                    ...
                    'tax_fiscal_country_id': doc.company_id.country_id.id,
            ...
            if key not in tax_cache:
                tax_cache[key] = self.env['account.tax'].search([
                    ('name', '=', tax_name),
                    ('company_id', '=', doc.company_id.id),
                ]) or self.env['account.tax'].sudo().create({
                    'name': tax_name,
                    'company_id': doc.company_id.id,
