odoo.define("documents_spreadsheet.spreadsheet_template_tests", function (require) {
    ...
    const { normalize } = require("documents_spreadsheet.spreadsheet");
    ...
    const { parse, astToFormula, Model } = spreadsheet;
    ...
    async function convertFormula(config) {
        ...
        const {text, dependencies} = normalize(config.formula);
        const ast = config.convertFunction(parse(text), dependencies, pivots);
        actionManager.destroy();
        return astToFormula(ast, dependencies);
    }
    ...
    QUnit.module(
        ...
        function () {
            ...
            QUnit.test("Adapt pivot as function arg from absolute to relative", async function (
                ...
                const { text, dependencies } = normalize(`
                    SUM(
                        PIVOT("1","probability","product_id","37","bar","110"),
                        PIVOT("1","probability","product_id","41","bar","110")
                    )
                `)
                let ast = parse(text);
                ast = pivotUtils.absoluteToRelative(ast, dependencies, pivots);
                assert.equal(
            ...
            QUnit.test("Computed ids are not changed", async function (assert) {
                ...
                const { text, dependencies } = normalize(`PIVOT("1","probability","product_id",A2,"bar","110")`);
                let ast = parse(text);
                ast = pivotUtils.absoluteToRelative(ast, dependencies, pivots);
                assert.equal(
                    astToFormula(ast, dependencies),
            ...
            QUnit.test("Autofill template position", async function (assert) {
                ...
                // TOTAL HEADER RIGHT
                // Select A5: Total header
                model.dispatch("SET_SELECTION", {
                    anchor: [0, 4],
                    zones: [{ top: 4, bottom: 4, right: 0, left: 0 }],
                });
                model.dispatch("AUTOFILL_SELECT", { col: 2, row: 4 });
                model.dispatch("AUTOFILL");
                assert.equal(
                    model.getters.getCell(1, 4).content,
                    `=PIVOT("1","probability","bar","110")`
                );
                assert.equal(
                    model.getters.getCell(2, 4).content,
                    `=PIVOT("1","probability")`
                );
            ...
            QUnit.test("User with readonly access cannot save spreadsheet template", async function (assert) {
                assert.expect(5);
                const actionManager = await createActionManager({
                    data: this.data,
                    mockRPC: async function (route, args) {
                        if (args.method === "check_spreadsheet_access" && args.model === "spreadsheet.template") {
                            assert.step("check_spreadsheet_access");
                            assert.deepEqual(args.args[0], 1, "It should write the correct document");
                            assert.deepEqual(args.args[1], 'write', "We are checking for write access");
                            return false;
                        }
                        return this._super.apply(this, arguments);
                    },
                ...
                await nextTick();
                assert.verifySteps(["check_spreadsheet_access", "notification"]);
                actionManager.destroy();
            });
            ...
            QUnit.test("Create new spreadsheet from template containing non Latin characters", async function (assert) {
                assert.expect(1);
            ...
            QUnit.test("open template with non Latin characters", async function (assert) {
                ...
                const template = o_spreadsheet.__DEBUG__.model // TODO make proper test helpers to open a spreadsheet
                assert.equal(
                    template.getters.getCell(0, 0).content, "😃",
                    "It should show the smiley as a smiley 😉"
                )
                actionManager.destroy();
            });

            QUnit.test('create and edit template and create new spreadsheet from it', async function (assert) {
                assert.expect(7);
                ...
                const [actionManager, model, env] = await createSpreadsheetActionManager({
                    id,
                    data: this.data,
                    spreadsheetAction: 'action_open_template',
                    mockRPC: function (route, args) {
                        if (args.model == 'spreadsheet.template'){
                            if (args.method === 'write') {
                                assert.step('write_template');
                ...
                const saveAsTemplate = topbarMenuRegistry.get('file').children.find(item => item.id == 'save');
                saveAsTemplate.action(env);
                await actionManager.doAction({
                    type: 'ir.actions.client',
                    tag: 'action_open_template',
                ...
                assert.verifySteps(['write_template', 'write_template']);
                actionManager.destroy();
            });
        },
    );
