...
QUnit.module("documents_spreadsheet > graph view", { beforeEach }, () => {
    ...
    QUnit.test("'cumulated_start' is fetched from the graph view", async (assert) => {
        const serverData = getBasicServerData();
        ...
        assert.strictEqual(model.getters.getChart(chartIds[0]).metaData.cumulatedStart, true);
    })
});
