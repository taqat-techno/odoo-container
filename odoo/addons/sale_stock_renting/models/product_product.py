...
class ProductProduct(models.Model):
    ...
    def _get_active_rental_lines(self, fro, to, **kwargs):
        ...
        if not to or fro == to:
            active_lines_at_time_fro = Reservation.search(domain + [
                ...
                ('return_date', '>' if kwargs.get('rental_pivot_date') else '>=', fro)
            ])
    ...
    def _get_virtual_unavailable_qty_in_rent(self, pivot_date, **kwargs):
        ...
        begins_during_period, ends_during_period, covers_period = self._get_active_rental_lines(
            fro=pivot_date, to=pivot_date, **kwargs
        )
        all_lines = (ends_during_period - begins_during_period) | covers_period
        all_lines = all_lines.filtered(lambda line:
        ...
        return sum(line.product_uom_qty for line in all_lines)
