...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_edi_check_configuration(self, move):
        ...
        if move.l10n_mx_edi_cfdi_request in ('on_invoice', 'on_refund'):
            if move.invoice_line_ids.filtered(lambda line: line.price_subtotal < 0):
    ...
    def _l10n_mx_edi_get_invoice_line_cfdi_values(self, invoice, line):
        ...
        tax_objected = invoice._l10n_mx_edi_get_tax_objected()
        if line.discount != 100.0:
            if tax_objected == '02':
                gross_price_subtotal = invoice.currency_id.round(line.price_subtotal / (1 - line.discount / 100.0))
            else:
                gross_price_subtotal = invoice.currency_id.round(line.price_total / (1 - line.discount / 100.0))
        else:
        ...
        if tax_objected == '02':
            ...
            cfdi_values['discount_amount'] = gross_price_subtotal - line.price_total
        ...
        for tax_res in tax_details['taxes']:
            ...
            cfdi_values['tax_details'].setdefault(tax, {
                ...
                'tax_amount': tax.amount / 100.0 if tax.amount_type != 'fixed' else tax_res['amount'] / tax_res['base'],
                'tax_name': tax_name,
                'total': 0.0,
            })
    ...
    def _l10n_mx_edi_get_invoice_cfdi_values(self, invoice):
        ...
        if invoice.currency_id.name == 'MXN':
            ...
            cfdi_values['currency_conversion_rate'] = abs(invoice.amount_total_signed) / abs(invoice.amount_total) if invoice.amount_total else 1
        ...
        for vals in cfdi_values['invoice_line_values']:
            for tax_res in vals['tax_details_transferred']:
                if tax_res['tax'].l10n_mx_tax_type == 'Cuota':
                    tax_res['tax_amount'] = tax_res['total'] / tax_res['base']
        for tax_res in cfdi_values['tax_details_transferred']:
            if tax_res['tax'].l10n_mx_tax_type == 'Cuota':
                tax_res['tax_amount'] = tax_res['total'] / tax_res['base']
    ...
    def _l10n_mx_edi_get_payment_cfdi_values(self, move):
        ...
        def divide_tax_details(invoice, filter_to_apply, amount_paid):
            tax_details = invoice._prepare_edi_tax_details(filter_to_apply)
        ...
        for field1, field2 in (('debit', 'credit'), ('credit', 'debit')):
            for partial in pay_rec_lines[f'matched_{field1}_ids']:
                ...
                # for CFDI 4.0
                tax_details_transferred = divide_tax_details(invoice, filter_tax_transferred, amount_paid_invoice_curr)
                tax_details_withholding = divide_tax_details(invoice, filter_tax_withholding, amount_paid_invoice_curr)
