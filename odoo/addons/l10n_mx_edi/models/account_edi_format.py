...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_edi_get_invoice_cfdi_values(self, invoice):
        ...
        mexico_tz = invoice._l10n_mx_edi_get_cfdi_partner_timezone(issued_address)
        timezoned_now = datetime.now(mexico_tz)
        timezoned_today = timezoned_now.date()
        if invoice.invoice_date >= timezoned_today:
            cfdi_date = timezoned_now.strftime(CFDI_DATE_FORMAT)
        else:
...
""" % {'boundary': boundary, 'cfdi_b64': cfdi_b64}
    ...
    def _l10n_mx_edi_get_sat_status(self, supplier_rfc, customer_rfc, total, uuid):
        ...
        params = "<![CDATA[?id=%s&re=%s&rr=%s&tt=%s]]>" % (
            uuid or '',
