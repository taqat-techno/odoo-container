...
class AccountMove(models.Model):
    ...
    def button_draft(self):
        for move in self:
            ...
            timezoned_today = move._l10n_mx_edi_get_datetime_now_with_mx_timezone(cfdi_values).date()
    ...
    def _l10n_mx_edi_get_min_of_now_and_document_date(self, issued_address):
        """ This method returns the lesser of the document date and the current time in the
        Mexican timezone determined by the issued address and the invoice's journal. """
        cfdi_values = {'issued_address': issued_address}
        now_mx = self._l10n_mx_edi_get_datetime_now_with_mx_timezone(cfdi_values).replace(tzinfo=None)
        document_datetime = datetime.combine(self.invoice_date, time(hour=23, minute=59, second=00))
    ...
    def _l10n_mx_edi_add_invoice_cfdi_values(self, cfdi_values, percentage_paid=None, global_invoice=False):
        ...
        if self.l10n_mx_edi_post_time:
            cfdi_date = self.l10n_mx_edi_post_time
        else:
            cfdi_date = self._l10n_mx_edi_get_min_of_now_and_document_date(self.company_id.partner_id.commercial_partner_id)
    ...
    def _l10n_mx_edi_cfdi_invoice_try_send(self):
        ...
        if not self.l10n_mx_edi_post_time:
            self.l10n_mx_edi_post_time = self._l10n_mx_edi_get_min_of_now_and_document_date(self.company_id.partner_id.commercial_partner_id)
