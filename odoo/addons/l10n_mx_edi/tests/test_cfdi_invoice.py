...
class TestCFDIInvoice(TestMxEdiCommon):
    ...
    def test_cfdi_rounding_12(self):
        ...
        def run(rounding_method):
            with self.mx_external_setup(self.frozen_today):
                ...
                with self.with_mocked_pac_sign_success():
                    invoice._l10n_mx_edi_cfdi_invoice_try_send()
                self._assert_invoice_cfdi(invoice, f'test_cfdi_rounding_12_{rounding_method}_inv')

                payment = self._create_payment(invoice, currency_id=self.usd.id)
                with self.with_mocked_pac_sign_success():
                    payment.move_id._l10n_mx_edi_cfdi_payment_try_send()
                self._assert_invoice_payment_cfdi(payment.move_id, f'test_cfdi_rounding_12_{rounding_method}_pay')

        rate = 1 / 20.4277
        self.setup_rates(self.usd, (self.frozen_today, rate))
        self._test_cfdi_rounding(run)
    ...
    def test_full_payment_rate(self):
        ...
        self.setup_rates(self.usd, (date1, 0.049905678268), (date2, 0.049073733284))
    ...
    def test_sw_finkok_CRP20211_usd_statement_in_mxn_journal_rounded_exchange_rate(self):
        ...
        # Rates for how much USD for 1 MXN
        self.setup_rates(self.usd, (payment_date, 0.05))
        ...
        with self.mx_external_setup(payment_date):
            ...
            receivable_line = invoice1.line_ids.filtered(lambda l: l.account_id.account_type == 'asset_receivable')
