...
class GenericTaxReportCustomHandler(models.AbstractModel):
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        if options.get('unposted_in_period'):
            # Recompute the option 'unposted_in_period' to not include tax closing entries in the banner of unposted moves
            options['unposted_in_period'] = bool(self.env['account.move'].search_count(
                [('state', '=', 'draft'), ('date', '<=', options['date']['date_to']), ('tax_closing_end_date', '=', False)],
                limit=1,
            ))
            ...
            for column_group in options['column_groups']:
                ...
                if options.get('account_journal_report_tax_deductibility_columns'):
                    for deduct_type in ('tax_non_deductible', 'tax_deductible', 'tax_due'):
                        columns.append({
                            'no_format': sign * tax_amount_dict[deduct_type][column_group],
                            'name': report.format_value(
                                sign * tax_amount_dict[deduct_type][column_group],
    ...
    def action_periodic_vat_entries(self, options):
        ...
        draft_closing_moves = self._get_tax_closing_entries_for_closed_period(report, options, non_tax_locked_companies, posted_only=False) + draft_locked_moves
