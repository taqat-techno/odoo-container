...
class BankReconciliationReportCustomHandler(models.AbstractModel):
    ...
    def _report_custom_engine_misc_operations(self, expressions, options, date_scope, current_groupby, next_groupby, offset=0, limit=None, warnings=None):
        ...
        misc_operations = self.env["account.move.line"].search_read(
            domain=bank_miscellaneous_domain or [],
            fields=['amount_currency', 'balance', 'date'],
        )
        misc_operations_amount = sum(
            company_currency._convert(misc_operation['balance'], journal_currency, journal.company_id, misc_operation['date'])
            for misc_operation in misc_operations
        )
