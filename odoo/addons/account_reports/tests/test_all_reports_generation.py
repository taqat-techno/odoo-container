            ...
            with self.subTest(report=report.name, country=report.country_id.name):
                ...
                for option_button in options['buttons']:
                    ...
                    with self.subTest(button=option_button['name']):
                        if option_button.get('action') == 'export_file' and option_button.get('action_param') == 'export_to_pdf':
                            # since _get_html_data_for_pdf_export is a generator, we need to convert it to a list to exhaust it
                            list(report._get_html_data_for_pdf_export(options))
                            continue
