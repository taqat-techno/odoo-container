...
class TestPickingBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_internal_picking_from_scratch_with_package(self):
        ...
        self.start_tour(url, 'test_internal_picking_from_scratch_with_package', login='admin')
    ...
    def test_delivery_pack_from_different_location(self):
        ...
        url = "/web#action=" + str(action_main_menu.id)
        self.start_tour(url, 'test_delivery_pack_from_different_location', login='admin')

    def test_delivery_scan_unreserved_lot_as_subline(self):
        """ Ensures that scanning a non-reserved lot for a reserved product will
        create a subline for this product's barcode line instead of creating a
        new barcode line, even when the focus is on another product's line.
        """
        self.clean_access_rights()
        grp_lot = self.env.ref('stock.group_production_lot')
        self.env.user.write({'groups_id': [Command.link(grp_lot.id)]})
        lot1 = self.env['stock.lot'].create({'name': 'Lot0001', 'product_id': self.productlot1.id, 'company_id': self.env.company.id})  # The one reserved
        lot2 = self.env['stock.lot'].create({'name': 'Lot0002', 'product_id': self.productlot1.id, 'company_id': self.env.company.id})  # The one scanned
        self.env['stock.quant']._update_available_quantity(self.productlot1, self.stock_location, 5, lot_id=lot1)
        self.env['stock.quant']._update_available_quantity(self.productlot1, self.stock_location, 5, lot_id=lot2)
        self.env['stock.quant']._update_available_quantity(self.product1, self.stock_location, 5)

        # Create a delivery for two products, with at least one tracked product.
        delivery_picking = self.env['stock.picking'].create({
            'location_id': self.stock_location.id,
            'location_dest_id': self.customer_location.id,
            'picking_type_id': self.picking_type_out.id,
        })
        lot_move = self.env['stock.move'].create({  # The one affected by the scan
            'name': 'test_delivery_lot1',
            'location_id': self.stock_location.id,
            'location_dest_id': self.customer_location.id,
            'product_id': self.productlot1.id,
            'product_uom': self.uom_unit.id,
        ...
        self.env['stock.move'].create({  # The one selected
            'name': 'test_delivery_common',
            'location_id': self.stock_location.id,
            'location_dest_id': self.customer_location.id,
            'product_id': self.product1.id,
            'product_uom': self.uom_unit.id,
        ...
        delivery_picking.action_confirm()
        lot_move.move_line_ids.lot_id = lot1

        url = self._get_client_action_url(delivery_picking.id)
        self.start_tour(url, 'test_delivery_scan_unreserved_lot_as_subline', login='admin')
