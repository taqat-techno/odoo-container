...
class TestPickingBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_barcode_pack_lot(self):
        """
        This test ensures that products of the same not-reserved lot can be
        ...
        self.env['stock.quant']._update_available_quantity(product, warehouse.lot_stock_id, 2, lot_id=lot_1)
        ...
        self.assertEqual(delivery.state, 'done')
        self.assertRecordValues(delivery.move_line_ids, [
            {'qty_done': 1.0, 'lot_id': lot_2.id},
            {'qty_done': 1.0, 'lot_id': lot_2.id},
        ])
        self.assertEqual(len(delivery.move_line_ids.result_package_id), 2)
    ...
    def test_gs1_tracked_packaging(self):
        ...
        self.assertRecordValues(move_lines, [
            {'qty_done': 6, 'lot_id': lot.id, 'location_id': self.supplier_location.id, 'location_dest_id': self.stock_location.id},
            {'qty_done': 6, 'lot_id': lot.id, 'location_id': self.stock_location.id, 'location_dest_id': self.customer_location.id},
        ])

    def test_gs1_tracked_lot(self):
        """
        Ensures we can scan a GS1 barcode containing a lot_name
        """
        self.clean_access_rights()
        self.env.company.nomenclature_id = self.env.ref('barcodes_gs1_nomenclature.default_gs1_nomenclature')
        group_tracking = self.env.ref('stock.group_production_lot')
        self.env.user.write({'groups_id': [(4, group_tracking.id, 0)]})
        product = self.productlot1
        lot = self.env['stock.lot'].create({
            'name': '10032324016',
            'product_id': product.id,
            'company_id': self.env.company.id
        ...
        action_id = self.env.ref('stock_barcode.stock_barcode_action_main_menu')
        url = "/web#action=" + str(action_id.id)
        self.start_tour(url, 'test_gs1_tracked_lot', login='admin', timeout=180)
