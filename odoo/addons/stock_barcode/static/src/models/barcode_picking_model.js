...
export default class BarcodePickingModel extends BarcodeModel {
    ...
    getIncrementQuantity(line) {
        ...
        const quantityToFormat = Math.max(remainingQty, 1);
    ...
    _lineIsNotComplete(line) {
        const current_line = this._getParentLine(line) || line;
        return current_line.product_uom_qty && current_line.qty_done < current_line.product_uom_qty;
    }
    ...
    async _processBarcode(barcode) {
        if (this.isDone && !this.commands[barcode]) {
            return this.notification.add(_t("This picking is already done"), { type: "danger" });
        }
    ...
    async _processPackage(barcodeData) {
        ...
        if (barcodeData.packageType && !recPackage) {
            ...
            recPackage.location_id && ![page.sourceLocationId, page.destinationLocationId].includes(recPackage.location_id)
        )) {
        ...
        if ((!quants.length || page.destinationLocationId === recPackage.location_id) &&
            (currentLine && !currentLine.result_package_id)) {
        ...
        if (page.sourceLocationId !== recPackage.location_id) {
        ...
        for (const quant of quants) {
            ...
            while (remaining_qty > 0) {
                ...
                if (currentLine) { // Updates an existing line.
                    const qty_needed = Math.max(currentLine.product_uom_qty - currentLine.qty_done, 0);
