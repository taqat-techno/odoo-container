...
class TestAccountOnlineAccount(AccountOnlineSynchronizationCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.bank_account_id = cls.env['account.account'].create({
            ...
            'code': cls.env['account.account']._search_new_account_code(cls.company_data['company'], digits=6, prefix='BNK'),
        })
    ...
    @patch("odoo.addons.account_online_synchronization.models.account_online.AccountOnlineLink._get_consent_expiring_date", return_value=None)
    def test_assign_journal_with_currency_on_account_online_account(self, patched_get_consent_expiring_date):
    ...
    @patch("odoo.addons.account_online_synchronization.models.account_online.AccountOnlineLink._get_consent_expiring_date", return_value=None)
    def test_set_currency_on_journal_when_existing_currencies_on_move_lines(self, patched_get_consent_expiring_date):
        bank_account_id = self.env['account.account'].create({
            'name': 'Bank Account',
            'account_type': 'asset_cash',
            'code': self.env['account.account']._search_new_account_code(self.company_data['company'], digits=6, prefix='BNK'),
        })
